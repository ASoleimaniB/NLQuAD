        ...
        for paragraph in example["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                tensors_list.append((torch.tensor(input_ids_list), torch.tensor(input_mask_list),
                                     ...
                                     qa['id'],  [qa['answers'][0]['text']]))  # for eval # self._get_qid(qa['id']),  qa["aliases"]))  # for eval
        return tensors_list
...
class TriviaQA(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.args.save_prefix=args.save_prefix
    ...
    def test_end(self, outputs):
        ...
        with open(self.args.save_prefix+'/predictions.json', 'w') as f:
    ...
    def configure_optimizers(self):
        def lr_lambda(current_step):
            ...
            # return max(0.0, float(self.args.steps - current_step) / float(max(1, self.args.steps - self.args.warmup))) # global step issue for lightning 0.6.0
            return max(0.0, float((self.args.steps*self.args.batch_size) - current_step) / float(max(1, (self.args.steps*self.args.batch_size) - self.args.warmup)))
    ...
    def train_dataloader(self):
        ...
        dataset = SQuADDataset(file_path=self.args.train_dataset, tokenizer=self.tokenizer,
        ...
        dl = DataLoader(dataset, batch_size=1, shuffle=(sampler is None),
                        ...
                        collate_fn=SQuADDataset.collate_one_doc_and_lists)
    ...
    def val_dataloader(self):
        ...
        dataset = SQuADDataset(file_path=self.args.dev_dataset, tokenizer=self.tokenizer,
                                  max_seq_len=self.args.max_seq_len, max_doc_len=self.args.max_doc_len,
                                  doc_stride=self.args.doc_stride,
                                  max_num_answers=self.args.max_num_answers,
                                  max_question_len=self.args.max_question_len,
...
def main(args):
    ...
    print(args)
    train_set_size = L_mydata#10#110648  # hardcode dataset size. Needed to compute number of steps for the lr scheduler
